		...
		$('.tj').click(function(){
			...
			if(mm && xmm){
				if(mm != xmm){
					...
					var pattern = /^[\w_-]{6}$/;
					if(!pattern.test(mm)){
						$('.ts').html('支付密码必须是6位').css('color','red');
						return false;
					}else{
...
<div class="wy-ay-pn2">
	...
	<div><input type="password" placeholder="&nbsp;&nbsp;&nbsp;请输入支付密码" class="mm"></div>
	<div>
		<form action="{{url('/payupdate2')}}" method="post">
			<input type="hidden" name="_token" value="{{csrf_token()}}"> 
			<input type="password" placeholder="&nbsp;&nbsp;&nbsp;请确认支付密码" class="xmm" name="password">
